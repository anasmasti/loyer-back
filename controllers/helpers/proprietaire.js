      ...
      if (contrat.foncier.lieu[g].deleted == false) {
        for (let j = 0; j < contrat.proprietaires.length; j++) {
          if (contrat.proprietaires[j].is_mandataire == true) {
            ...
            // let dureeAvance = contrat.duree_avance;
            let montantAvance = 0;
            let taxAvance = 0;

            montantAvance =
              +contrat.proprietaires[j].montant_loyer.toFixed(2) * dureeAvance;
            taxAvance =
              +contrat.proprietaires[j].retenue_source.toFixed(2) * dureeAvance;
