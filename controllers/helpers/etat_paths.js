...
async function storePaths(mois, annee, extention, dateToString, etatType) {
  let reportingPaths = [];
  await etatPaths
    .find({
      mois: mois,
    ...
    .then(async (data) => {
      try {
        if (data.length == 0) {
          ...
          let etat_pathsssssss = new etatPaths({
            etat_paths: reportingPaths,
            mois: mois,
            annee: annee,
          });
          return await etat_pathsssssss.save();
        } else {
          for (let i = 0; i < data[0].etat_paths.length; i++) {
            for (j in data[0].etat_paths[i]) {
              if (
                data[0].etat_paths[i][j] ==
                `download/generated situation/${etatType}_${extention}/${extention}/${etatType}_${dateToString}.${extention}`
              ) {
                reportingPaths.push({
                  [`${etatType}_${extention}`]: `download/generated situation/${etatType}_${extention}/${etatType}_${dateToString}.${extention}`,
                });
          ...
          if (pathExist == false) {
            reportingPaths.push({
              [`${etatType}_${extention}`]: `download/generated situation/${etatType}_${extention}/${extention}/${etatType}_${dateToString}.${extention}`,
              extention: extention,
            });
          }
