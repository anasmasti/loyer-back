...
module.exports = {
    setComptabilisationCautions: async (_, res) => {
        ...
        fs.writeFile('download/FichierComptableCaution ' + currentMonthName + ' ' + today.getFullYear() + '.txt', '', { flag: 'w' }, (err) => {
            if (err) throw err
        })
        ...
        Contrat.find().populate('lieu').populate('foncier').populate({ path: 'foncier', populate: { path: 'proprietaire' } })
            .then((data) => {
                ...
                for (let index = 0; index < data.length; index++) {
                    ...
                    fs.writeFileSync('download/FichierComptableCaution ' + currentMonthName + ' ' + today.getFullYear() + '.txt', ecritureDebiterCaution, { flag: "a" }, (err) => {
