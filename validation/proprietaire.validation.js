...
const ProprietaireValidation = Joi.object({
  ...
  cin: Joi.string().empty("").min(4).max(8).messages({
  ...
  passport: Joi.string().empty("").min(4).max(8).messages({
  ...
  carte_sejour: Joi.string().empty("").min(4).max(8).messages({
  ...
  n_registre_commerce: Joi.string().empty("").min(2).max(50).messages({
    ...
    "string.max":
      "Numéro de registre commerce doit être 50 charactères au maximum",
  }),
  telephone: Joi.string().empty("").min(1).max(10).messages({
    "string.max": "Téléphone doit être 10 chiffres au maximum",
  }),
  fax: Joi.string().empty("").min(1).max(10).messages({
  ...
  // }),
  taux_impot: Joi.number().empty().messages({
    "number.base": "Taux de l'impot contient juste des chiffres",
  }),
  retenue_source: Joi.number().empty(),
  montant_apres_impot: Joi.number().empty().messages({
    "number.base": "Montant apres l'impot contient juste des chiffres",
  }),
  nom_agence_bancaire: Joi.string().empty("").max(250).messages({
  ...
  montant_avance_proprietaire: Joi.number().empty().messages({
    "number.base": "Montant avance proprietaire contient juste des chiffres",
  }),
  tax_avance_proprietaire: Joi.number().empty().messages({
    "number.base": "Tax avance proprietaire contient juste des chiffres",
  }),
  tax_par_periodicite: Joi.number().empty().messages({
    "number.base": "Tax par periodicite contient juste des chiffres",
  }),
  part_proprietaire: Joi.number().empty().messages({
    "number.base": "Part proprietaire contient juste des chiffres",
    // pourcentage:
  }),
  caution_par_proprietaire: Joi.number().empty().messages({
  ...
  proprietaire_list: Joi.array().items(Joi.string()),
  old_proprietaires_list: Joi.array().items(Joi.string()),
  has_mandataire: Joi.string(),
  declaration_option: Joi.string().empty(""),
  is_person_physique: Joi.boolean(),
  type_proprietaire: Joi.string(),
});
